...
The guard is not a billing ceiling, hostile-process sandbox or account-wide policy. It does not cover arbitrary API calls or every way to start another session. The original tests establish bounded behavior on specific engines; see [validation](plugins/aperture-agent-system/docs/validation.md).
