...
Cost-aware delegation for Codex, with named specialist agents and an optional model guard. This standalone repository contains only the **Aperture-Agent-System** plugin and its documentation.
...
| Goal | Guide |
|---|---|
| Understand the package and install it | [Aperture quick start](plugins/aperture-agent-system/README.md) |
| Choose roles, toggle the guard, or add a custom agent | [Configuration](plugins/aperture-agent-system/docs/configuration.md) |
| Update, troubleshoot, or remove project assets | [Operations and recovery](plugins/aperture-agent-system/docs/operations.md) |
| Understand what has actually been tested | [Validation and compatibility](plugins/aperture-agent-system/docs/validation.md) |
| Contribute a change | [Contributing](CONTRIBUTING.md) |
| Report a security concern | [Security](SECURITY.md) |

## Install the plugin

First review the [client and model prerequisites](plugins/aperture-agent-system/README.md#requirements). These commands register the repository catalog and install its skill package:
...
Choose one catalog source. If that catalog is already registered, inspect `codex plugin marketplace list` before adding it again. Plugin installation makes three skills available; it does not apply project configuration, grant hook trust, or enable enforcement. Continue with the [project setup steps](plugins/aperture-agent-system/README.md#set-up-one-project).
...
| Plugin ID | `aperture-agent-system` |
| Package directory | `plugins/aperture-agent-system/` |
| Native catalog | `.agents/plugins/marketplace.json` |

The plugin keeps its existing ID so its skills and project markers stay consistent. This repository contains no sibling plugins from the original multi-plugin repository. Its new Git history starts from the selected Aperture package files.
